...
function resolveGraphic(detail, courseSlug) {
  ...
  if (matched) {
    return `/graphics/${courseSlug}/${encodeURIComponent(matched)}`;
  }
...
function runtimeContent(content, relativeFile, courseSlug, tikzManifest) {
  ...
  return withDiagrams.replace(/^:::figure\s+([^\n]+)\r?\n:::/gm, (source, detail) => {
    const url = resolveGraphic(detail, courseSlug);
...
function titleOf(source, fallback) {
  const raw = cleanTitle(source.match(/\\title(?:\[[^\]]*\])?\{([^}]*)\}/)?.[1] || fallback);
  return raw.replace(/^(?:Lecture|คาบเรียนที่)\s*\d+\s*[:–-]?\s*/i, "").replace(/\\\\/g, " ").trim();
}
...
const coursesToBuild = [
  {
    slug: "calculus-1-for-engineers",
    srcDir: "C:/Users/sakul_bp6myy0/OneDrive/Tex/Slides/Calculus 1 for Engineers",
    outFile: "course-data-calc1.json"
  },
  {
    slug: "calculus-2-for-engineers",
    srcDir: "C:/Users/sakul_bp6myy0/OneDrive/Tex/Slides/Calculus 2 for Engineers",
    outFile: "course-data-calc2.json"
  },
  {
    slug: "calculus-3-for-engineers",
    srcDir: "C:/Users/sakul_bp6myy0/OneDrive/Tex/Slides/Calculus 3 for Engineers",
    outFile: "course-data-calc3.json"
  }
];

for (const course of coursesToBuild) {
  const contentRoot = path.join(root, "content", "courses", course.slug, "lectures");
  const manifestFile = path.join(root, "content", "courses", course.slug, "tikz-manifest.json");
  ...
  for (let week = 1; week <= 15; week += 1) {
    ...
    const lectureFile = path.join(course.srcDir, `lecture${id}.tex`);
    const worksheetFile = path.join(course.srcDir, `worksheet${id}.tex`);
    ...
    const generated = (rawSections.length ? rawSections : [{ title: "Overview", preferredSlug: "overview", content: texToMarkdown(lectureSource) }]).map((section, index) => {
      ...
      const relativeFile = `content/courses/${course.slug}/lectures/${id}/${filename}`;
      const processedContent = runtimeContent(parsed.content, relativeFile, course.slug, tikzManifest);
      return {
  ...
  fs.writeFileSync(path.join(root, "src", "lib", course.outFile), JSON.stringify(weeks, null, 2));
  console.log(`[${course.slug}] Successfully generated ${weeks.length} weeks -> src/lib/${course.outFile}`);
}
