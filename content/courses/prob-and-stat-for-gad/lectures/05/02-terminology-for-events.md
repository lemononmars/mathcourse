...
- $B$ and $D$ are mutually exclusive. You can tell without listing all the elements because if both dice have the same value, then the different must be 0, not 3. 

- On the other hand, $A \cap B = \{11\}$ and $B \cap C = \{44\}$, so neither pair $A$ and $B$, or $B$ and $C$ are mutually exclusive.

- $A$ and $C$ are mutually exclusive. If the first dice is 1 and the sum is 8, then the other dice would need to be 7, which is not possible.
