...
%%%%%%%%%%%%%
\section[Expressions]{Algebraic Expressions}
\frame{\sectionpage}
%%%%%%%%%%%%%

\begin{frame}{Algebraic Expressions}
\begin{defn}{Algebraic Expression}{}
An \textbf{algebraic expression} is a mathematical phrase composed of numbers (constants), variables, and arithmetic operations ($+$, $-$, $\times$, $\div$, powers).
\end{defn}
...
\item $3x + 5$
\item $2x^2 - 4x + 7$
\item $\displaystyle \frac{x + 3}{2x - 1}$
\item $4\sqrt{x} - 2y + 8$
\end{itemize}
\end{ex}
\end{frame}

\begin{frame}{Components of an Algebraic Expression}
\begin{defn}{Components of an Algebraic Expression}{}
\begin{itemize}
\item \textbf{Variable:} A symbol representing an unknown value (e.g., $x, y, t$).
\item \textbf{Constant:} A fixed numeric value with no variable attached (e.g., $4, -7, \frac{2}{3}$).
\item \textbf{Coefficient:} The numerical factor multiplying a variable (in $5x$, $5$ is the coefficient).
\end{itemize}
\end{defn}
\end{frame}

\begin{frame}{Example: Identifying Components}
\begin{ex}
Consider the algebraic expression:
\[ 4x^2 - 7x + 9 \]
Determine the variable, coefficients, and constant term.
\end{ex}
\begin{sol}
\begin{itemize}
\item \textbf{Variable:} $x$
\item \textbf{Coefficients:} $4$ (for $x^2$) and $-7$ (for $x$)
\item \textbf{Constant term:} $9$
\end{itemize}
\end{sol}
\end{frame}

\begin{frame}{Terms and Like Terms}
\begin{defn}{Terms and Like Terms}{}
\begin{itemize}
\item \textbf{Term:} A single number, variable, or numbers and variables multiplied together, separated by $+$ or $-$ signs (in $3x^2 - 5x + 7$, terms are $3x^2, -5x, 7$).
\item \textbf{Like Terms:} Terms sharing identical variable parts with the same exponents (e.g., $4x$ and $-2x$).
\item \textbf{Unlike Terms:} Terms that have different variables or different powers of the same variable (e.g., $5x^2$ and $5x$).
\end{itemize}
\end{defn}
\end{frame}

\begin{frame}{Example: Terms and Like/Unlike Terms}
\begin{ex}
\begin{itemize}
\item In $5x^3 - 2x + 8$, the \textbf{terms} are $5x^3$, $-2x$, and $8$.
\item $3x$ and $-8x$ are \textbf{like terms} (same variable $x$).
\item $5x^2$ and $5x$ are \textbf{unlike terms} (different powers of $x$).
\end{itemize}
\end{ex}
\end{frame}

\begin{frame}{Fundamental Algebraic Properties (1/5)}
\begin{prop}{Commutative Properties}{}
For all real numbers $a, b$:
\begin{itemize}
\item \textbf{Commutative Property of Addition:} Changing order does not change the sum.
\[ a + b = b + a \]
\item \textbf{Commutative Property of Multiplication:} Changing order does not change the product.
\[ a \cdot b = b \cdot a \]
\end{itemize}
\end{prop}
\end{frame}

\begin{frame}{Fundamental Algebraic Properties (2/5)}
\begin{prop}{Associative Properties}{}
For all real numbers $a, b, c$:
\begin{itemize}
\item \textbf{Associative Property of Addition:} Changing grouping does not change the sum.
\[ (a + b) + c = a + (b + c) \]
\item \textbf{Associative Property of Multiplication:} Changing grouping does not change the product.
\[ (a \cdot b) \cdot c = a \cdot (b \cdot c) \]
\end{itemize}
\end{prop}
\end{frame}

\begin{frame}{Fundamental Algebraic Properties (3/5)}
\begin{prop}{Distributive Property}{}
For all real numbers $a, b, c$:
\begin{itemize}
\item \textbf{Left Distributive Property:}
\[ a(b + c) = ab + ac \]
\item \textbf{Right Distributive Property:}
\[ (a + b)c = ac + bc \]
\end{itemize}
\textit{Description:} Multiplying a number by a sum distributes multiplication to each addend inside the parentheses.
\end{prop}
\end{frame}

\begin{frame}{Fundamental Algebraic Properties (4/5)}
\begin{prop}{Identity Properties}{}
For any real number $a$:
\begin{itemize}
\item \textbf{Additive Identity:} Adding zero leaves the number unchanged ($0$ is the additive identity).
\[ a + 0 = a \]
\item \textbf{Multiplicative Identity:} Multiplying by one leaves the number unchanged ($1$ is the multiplicative identity).
\[ a \cdot 1 = a \]
\end{itemize}
\end{prop}
\end{frame}

\begin{frame}{Fundamental Algebraic Properties (5/5)}
\begin{prop}{Inverse Properties}{}
For any real number $a$:
\begin{itemize}
\item \textbf{Additive Inverse:} Adding the opposite (negation) yields zero.
\[ a + (-a) = 0 \]
\item \textbf{Multiplicative Inverse:} Multiplying by the reciprocal yields one (for $a \neq 0$).
\[ a \cdot \frac{1}{a} = 1 \]
\end{itemize}
\end{prop}
\end{frame}

\begin{frame}[t]{Simplifying Algebraic Expressions}
\begin{ex}
Simplify the following algebraic expressions by expanding and combining like terms.
\begin{tasks}(2)
\task $3x + 5y - 7x + 2y$
\task $2(3x - 4) + 4(x + 5)$
\end{tasks}
...
\begin{sol}
\begin{eqnarray*}
a) \quad 3x + 5y - 7x + 2y &=& (3x - 7x) + (5y + 2y) \\
&=& -4x + 7y \\[6pt]
b) \quad 2(3x - 4) + 4(x + 5) &=& (6x - 8) + (4x + 20) \\
&=& (6x + 4x) + (-8 + 20) \\
&=& 10x + 12
\end{eqnarray*}
\end{sol}
...
\begin{frame}{Properties that Preserve Equations}
\begin{prop}{Properties of Equality}{}
Let $A, B$ be expressions and $c$ be a real number. If $A = B$, then:
\begin{itemize}
\item \textbf{Addition Property:} $A + c = B + c$
\item \textbf{Subtraction Property:} $A - c = B - c$
\item \textbf{Multiplication Property:} $A \cdot c = B \cdot c$
\item \textbf{Division Property:} $\frac{A}{c} = \frac{B}{c} \quad (c \neq 0)$
\end{itemize}
\end{prop}
\end{frame}
...
\begin{frame}[t]
\begin{ex}
Solve the following linear equations using the distributive property.
\begin{tasks}(2)
\task $2(x - 3) = 4(x + 1)$
\task $3(2x + 1) - 4 = 2(x - 5) + 3$
\end{tasks}
\end{ex}
\begin{sol}
\begin{eqnarray*}
...
\begin{tasks}(2)
\task $3x - 7 = 11$
\task $\displaystyle \frac{2}{3}x + 4 = -2$
\task $5x + 3 = 2x - 9$
\task $4(x - 2) = 3(x + 5)$
\task $2(3x - 1) + 5 = 4(x + 3)$
\task $3x - 2(x + 4) = 5x + 6$
\task! $(2x + 1) + 3(x - 2) = 3x + 7$
\task! $4(2x - 3) - 2(x + 1) = 3(x + 4) - 5$
\end{tasks}
\end{exercisebox}
\end{frame}
...
\begin{frame}{Methods for Solving Systems of Equations}
There are two primary methods to solve a system of linear equations:
\begin{enumerate}
\item \textbf{Substitution Method:}
\begin{itemize}
\item Solve one equation for one variable in terms of the other (e.g., $y = \dots$).
\item Substitute this expression into the other equation to get a single-variable equation.
\item Solve for that variable, then substitute back to find the remaining variable.
\end{itemize}
\item \textbf{Elimination Method:}
\begin{itemize}
\item Multiply equations by appropriate constants so coefficients of a variable become opposites.
...
\begin{frame}[t]
\frametitle{Method 1: Substitution}
\begin{ex}
Solve the following system of equations using \textbf{substitution}.
\begin{eqnarray}
y &=& 2x - 1 \\
3x + 2y &=& 12
\end{eqnarray}
...
\begin{frame}[t]
\frametitle{Method 2: Elimination}
We can also solve a system of equations by \textbf{eliminating} a variable. Suppose that we have
\begin{ex}
\begin{eqnarray}
x+y &=& 4\\
x-y &=& 2
...
&=& (x-10)(x+2)
\end{eqnarray*}
Hence, the roots are $x = 10$ and $x=-2$.
\end{sol}
\end{frame}
