<script lang="ts">
  ...
  import type { CourseWeek } from '$lib/course'; import MathText from './MathText.svelte'; import SiteHeader from './SiteHeader.svelte'; import ThingsInRings from './ThingsInRings.svelte';
  let { courseWeek }:{courseWeek:CourseWeek}=$props();
</script>

<main class="min-h-screen bg-base-200/50"><SiteHeader compact/><div class="mx-auto max-w-6xl px-5 py-10">
  <a href="/courses/math-for-designer" class="btn btn-ghost btn-sm -ml-3 mb-6"><ArrowLeft size={16}/> Course</a>
  <header class="mb-8"><p class="eyebrow">Week {courseWeek.week} studio</p><h1 class="display-title mt-2 text-4xl font-black tracking-[-.04em] md:text-6xl">{courseWeek.title} activities</h1></header>
  {#if courseWeek.week===1}<ThingsInRings/>{/if}
