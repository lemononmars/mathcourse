<script lang="ts">
  ...
  import type { CourseWeek } from '$lib/course'; import MathText from './MathText.svelte'; import SiteHeader from './SiteHeader.svelte'; import ThingsInRings from './ThingsInRings.svelte'; import Week2InteractiveActivities from './Week2InteractiveActivities.svelte'; import Week3InteractiveActivities from './Week3InteractiveActivities.svelte';
...
<main class="min-h-screen bg-base-200/50"><SiteHeader compact/><div class="mx-auto max-w-6xl px-3 py-4 sm:px-5 sm:py-10">
  ...
  {:else if courseWeek.week===3}
    <Week3InteractiveActivities/>
  {/if}
  {#if courseWeek.activity.length && courseWeek.week > 3}
