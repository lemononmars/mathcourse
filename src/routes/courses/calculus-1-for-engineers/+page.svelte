<script lang="ts">
  ...
  import { calc1Weeks, weekAccent, lectureSectionPath } from '$lib/course';

  let weekProgress = $state<Record<number, number>>({});
  onMount(() => {
    weekProgress = Object.fromEntries(calc1Weeks.map((week) => {
      let completed: string[] = [];
      try { completed = JSON.parse(localStorage.getItem(`course-progress:calculus-1-for-engineers:${week.week}`) ?? '[]'); } catch { completed = []; }
...
<svelte:head>
  <title>Calculus 1 for Engineers · แคลคูลัส 1</title>
</svelte:head>
...
<main class="min-h-screen bg-base-200/50">
  ...
  <section class="border-b border-base-300 bg-base-100">
    <div class="mx-auto max-w-6xl px-6 py-14 md:py-20">
      <div class="badge badge-primary badge-outline mb-4 font-bold">MTH 101 · 15 สัปดาห์</div>
      <h1 class="display-title max-w-3xl text-4xl font-black tracking-[-.045em] md:text-6xl">Calculus 1 for Engineers<br/><span class="text-primary text-3xl md:text-5xl">แคลคูลัส 1 สำหรับวิศวกร</span></h1>
      <p class="mt-5 max-w-2xl text-lg leading-8 text-base-content/65">ฟังก์ชัน ลิมิตและความต่อเนื่อง อนุพันธ์ กฎลูกโซ่ การประยุกต์อนุพันธ์ ปริพันธ์จำกัดเขต และการหาพื้นที่/ปริมาตรการหมุน</p>
          ...
          <div class="min-w-0 flex-1">
            ...
            <div class="mt-4 flex flex-wrap gap-2">
              {#if week.sections.length}
                <a class="btn btn-sm rounded-full" href={lectureSectionPath('calculus-1-for-engineers', week)}>
                  บทเรียน <ArrowRight size={14}/>
                </a>
              {/if}
              <a class="btn btn-sm btn-ghost rounded-full" href={`/courses/calculus-1-for-engineers/activity/${week.week}`}>
                กิจกรรมสตูดิโอ
              </a>
              {#if week.worksheet?.length}
                <a class="btn btn-sm btn-ghost rounded-full" href={`/courses/calculus-1-for-engineers/worksheet/${week.week}`}>
                  แบบฝึกหัด
                </a>
              {/if}
            </div>
